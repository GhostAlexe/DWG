...
#include "ui_zdwgcontroldialog.h"
...
std::list<std::string> ZDwgControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDwgControlDialog::listMints;

ZDwgControlDialog::ZDwgControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDwgControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDwgControlDialog::~ZDwgControlDialog()
{
    delete ui;
}

void ZDwgControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDwgControlDialog::updateList()
{
...
void ZDwgControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDwgControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDwg_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDwgControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDwgControlDialog::GetSelectedMints()
{
...
void ZDwgControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
