...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::DWG, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (dwg.conf).");
            QMessageBox::warning(this, "DWG Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#3d3d3d; bgcolor:#5B4C7C;}\n a, span { font-family: monospace; }\n span.addr {color:#3E2BCE; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #transparent;}\n td.d0 {font-weight: bold; color:#3d3d3d;}\n h2, h3 { white-space:nowrap; color:#3E2BCE;}\n a { color:#3e2bce; text-decoration:none; }\n a.nav {color:#3E2BCE;}\n";
